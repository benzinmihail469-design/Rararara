-- ============================================================================
-- Dark Hub - Premium Loading Screen + GUI System (AMOLED Style)
-- Полностью самодостаточный скрипт с системой загрузки
-- ============================================================================
...
-- ============================================================================
-- СИСТЕМА ЗАГРУЗКИ (LOADING SYSTEM) - AMOLED стиль
-- ============================================================================

-- Родительский контейнер для загрузочного экрана (полный экран)
local LoadingScreen = Instance.new("Frame", DarkHub)
LoadingScreen.Name = "LoadingScreen"
LoadingScreen.Size = UDim2.new(1, 0, 1, 0)
LoadingScreen.BackgroundColor3 = Color3.fromRGB(0, 0, 0) -- Идеально черный
LoadingScreen.ZIndex = 1000

-- === ЦЕНТРАЛЬНЫЙ БЛОК С ИКОНКОЙ ===
local CenterBlock = Instance.new("Frame", LoadingScreen)
CenterBlock.Name = "CenterBlock"
CenterBlock.Size = UDim2.new(0, 250, 0, 350)
CenterBlock.AnchorPoint = Vector2.new(0.5, 0.5)
CenterBlock.Position = UDim2.new(0.5, 0, 0.5, -30)
CenterBlock.BackgroundTransparency = 1
CenterBlock.ZIndex = 1001

-- Пульсирующая иконка (круглая)
local IconContainer = Instance.new("Frame", CenterBlock)
...
IconImage.Size = UDim2.new(1, -20, 1, -20)
IconImage.Position = UDim2.new(0, 10, 0, 10)
IconImage.BackgroundTransparency = 1
IconImage.Image = "rbxassetid://" .. CustomIconID
IconImage.ScaleType = Enum.ScaleType.Fit
IconImage.ZIndex = 1003
local IconCorner2 = Instance.new("UICorner", IconImage)
IconCorner2.CornerRadius = UDim.new(1, 0)

-- Текст статуса загрузки (меняется динамически)
local StatusLabel = Instance.new("TextLabel", CenterBlock)
...
-- Процент загрузки (крупный)
local PercentBig = Instance.new("TextLabel", CenterBlock)
PercentBig.Name = "PercentBig"
PercentBig.Size = UDim2.new(1, 0, 0, 50)
PercentBig.Position = UDim2.new(0, 0, 0.65, 0)
PercentBig.Text = "0%"
PercentBig.Font = Enum.Font.FredokaOne
PercentBig.TextColor3 = Color3.fromRGB(255, 255, 255)
PercentBig.TextSize = 32
PercentBig.BackgroundTransparency = 1
PercentBig.ZIndex = 1001

-- Прогресс-бар (тонкая полоска)
local BarContainer = Instance.new("Frame", CenterBlock)
BarContainer.Name = "BarContainer"
BarContainer.Size = UDim2.new(1, -40, 0, 6)
BarContainer.Position = UDim2.new(0, 20, 0.82, 0)
BarContainer.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
BarContainer.BorderSizePixel = 0
BarContainer.ZIndex = 1001
...
BarFill.BackgroundColor3 = Color3.fromRGB(0, 191, 255) -- Неоново-голубой
BarFill.BorderSizePixel = 0
BarFill.ZIndex = 1002
local BarFillCorner = Instance.new("UICorner", BarFill)
BarFillCorner.CornerRadius = UDim.new(1, 0)

-- === ПУЗЫРЬКИ (BUBBLES) - анимированные кружки ===
...
BubbleContainer.ZIndex = 999

-- Создание пузырьков
local bubbles = {}
local bubbleCount = 12
local screenSize = Instance.new("Frame", LoadingScreen)
screenSize.Size = UDim2.new(1, 0, 1, 0)
screenSize.BackgroundTransparency = 1

for i = 1, bubbleCount do
	local bubble = Instance.new("Frame", BubbleContainer)
	bubble.Size = UDim2.new(0, math.random(20, 50), 0, math.random(20, 50))
	bubble.BackgroundColor3 = Color3.fromRGB(0, 191, 255)
	bubble.BackgroundTransparency = math.random(60, 90) / 100
	...
	bubble.Position = UDim2.new(x / 100, 0, y / 100, 0)
	
	-- Сохраняем данные для анимации
	bubbles[i] = {
		Object = bubble,
		SpeedX = (math.random() * 0.3 + 0.1) * (math.random(0, 1) == 0 and 1 or -1),
		SpeedY = (math.random() * 0.3 + 0.1) * (math.random(0, 1) == 0 and 1 or -1),
		X = x / 100,
		Y = y / 100,
...
-- Анимация пузырьков через RenderStepped
local bubbleConnection
bubbleConnection = RunService.RenderStepped:Connect(function(dt)
	for _, b in ipairs(bubbles) do
		if b.Object and b.Object.Parent then
			b.X = b.X + b.SpeedX * dt * 0.8
			b.Y = b.Y + b.SpeedY * dt * 0.8
			
			-- Отражение от границ
			if b.X > 1 then b.X = 0; b.X = b.X + b.SpeedX * dt * 0.8 end
			if b.X < 0 then b.X = 1; b.X = b.X - b.SpeedX * dt * 0.8 end
			if b.Y > 1 then b.Y = 0; b.Y = b.Y + b.SpeedY * dt * 0.8 end
			if b.Y < 0 then b.Y = 1; b.Y = b.Y - b.SpeedY * dt * 0.8 end
			
			b.Object.Position = UDim2.new(b.X, 0, b.Y, 0)
		end
	end
end)
...
pulseConnection = RunService.RenderStepped:Connect(function(dt)
	if IconContainer and IconContainer.Parent then
		currentScale = currentScale + (0.15 * dt * pulseDirection)
		if currentScale > 1.08 then
			currentScale = 1.08
			pulseDirection = -1
		elseif currentScale < 0.92 then
			currentScale = 0.92
			pulseDirection = 1
		end
		
		-- Используем UIScale для плавной пульсации
		local scaleObj = IconContainer:FindFirstChild("PulseScale")
		if not scaleObj then
			scaleObj = Instance.new("UIScale", IconContainer)
			scaleObj.Name = "PulseScale"
		end
		scaleObj.Scale = currentScale
	end
end)

-- Текстовая логика (зависимость от процента)
local statusTexts = {
	{min = 0, max = 25, text = "ЗАГРУЗКА ИНТЕРФЕЙСА"},
	{min = 26, max = 50, text = "ИНИЦИАЛИЗАЦИЯ МОДУЛЕЙ"},
	{min = 51, max = 75, text = "НАСТРОЙКА АНИМАЦИЙ"},
	{min = 76, max = 99, text = "ПОДГОТОВКА К ЗАПУСКУ"},
	{min = 100, max = 100, text = "ГОТОВО!"}
}

local function updateLoadingStatus(percent)
	for _, status in ipairs(statusTexts) do
		if percent >= status.min and percent <= status.max then
			StatusLabel.Text = status.text
			break
		end
	end
	PercentBig.Text = string.format("%d%%", percent)
...
task.spawn(function()
	...
	while progress < 100 do
		...
		-- Скорость загрузки: быстрее в начале, медленнее к концу
		local delay = 0.025 + (progress / 100) * 0.015
		task.wait(delay)
	end
	...
	-- Остановка анимаций пузырьков и пульсации
	if bubbleConnection then bubbleConnection:Disconnect(); bubbleConnection = nil end
	if pulseConnection then pulseConnection:Disconnect(); pulseConnection = nil end
	
	-- Ожидание 0.5 секунды перед скрытием
	task.wait(0.5)
	
	-- Плавное исчезновение загрузочного экрана
	local fadeOut = tween(LoadingScreen, {BackgroundTransparency = 1}, 0.5)
	
	-- Плавное исчезновение всех элементов внутри
	for _, child in ipairs(LoadingScreen:GetDescendants()) do
		if child:IsA("TextLabel") or child:IsA("Frame") and child ~= LoadingScreen then
...
local function LoadMainGUI()
	-- Здесь находится весь твой основной код интерфейса
	-- Я интегрирую его в виде функции, чтобы он запускался после загрузки
	
	-- ========================================================================
	-- ОСНОВНАЯ ЛОГИКА ИНТЕРФЕЙСА (ТВОЙ КОД)
	-- ========================================================================
	...
	-- Safe GUI Parent Resolution (уже есть выше)
	...
	ConfigInputStroke.Color = Color3.fromRGB(40, 40, 40)
