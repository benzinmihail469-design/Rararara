...
-- ============================================================
--  ОСНОВНОЙ КОД: СОЗДАНИЕ GUI В СТИЛЕ NEVERLOSE
-- ============================================================

if not getgenv().Library then
    error("Библиотека не загружена! Убедитесь, что вы запустили скрипт с библиотекой.")
end
...
local Window = Library:Window({
    ...
    Logo = "1l20959262762131" -- можно заменить на свою иконку
})

-- Функция для быстрого создания страниц
local function AddPage(name, icon)
    return Window:Page({ Name = name, Icon = icon })
end

-- Вкладки с иконками (подберите свои ID, если нужно)
local AimbotPage     = AddPage("Aimbot", "6031091379")
local RagebotPage    = AddPage("Ragebot", "6031091379")
local AntiAimPage    = AddPage("Anti Aim", "6031091379")
local LegitbotPage   = AddPage("Legitbot", "6031091379")
local VisualsPage    = AddPage("Visuals", "6031091379")
local PlayersPage    = AddPage("Players", "6031091379")
local WeaponPage     = AddPage("Weapon", "6031091379")
local GrenadesPage   = AddPage("Grenades", "6031091379")
local WorldPage      = AddPage("World", "6031091379")
local ViewPage       = AddPage("View", "6031091379")
local MiscPage       = AddPage("Miscellaneous", "122669828593160")
local MainPage       = AddPage("Main", "6031091379")
local InventoryPage  = AddPage("Inventory", "6031091379")
local ScriptsPage    = AddPage("Scripts", "6031091379")
local ConfigsPage    = AddPage("Configs", "6031091379")

-- ============================================================
--  ВКЛАДКА MISC (пример с секциями и элементами)
-- ============================================================
...
MovementSection:Toggle({ Name = "Auto Jump", Flag = "AutoJump", Default = false })
MovementSection:Toggle({ Name = "Auto Straf", Flag = "AutoStraf", Default = false })
MovementSection:Toggle({ Name = "Quick Stop", Flag = "QuickStop", Default = false })
MovementSection:Toggle({ Name = "Strafe Assist", Flag = "StrafeAssist", Default = false })
MovementSection:Toggle({ Name = "Auto Peek", Flag = "AutoPeek", Default = false })
MovementSection:Toggle({ Name = "Edge Jump", Flag = "EdgeJump", Default = false })
MovementSection:Toggle({ Name = "Infinity Duck", Flag = "InfinityDuck", Default = false })
MovementSection:Toggle({ Name = "Blockbot", Flag = "Blockbot", Default = false })

-- Секция Other
local OtherSection = MiscPage:Section({
    Name = "Other",
    Description = "Other settings",
    Icon = "6031091379",
    Side = 1
})
OtherSection:Toggle({ Name = "Anti Untrusted", Flag = "AntiUntrusted", Default = false })
OtherSection:Toggle({ Name = "Event Log", Flag = "EventLog", Default = false })
OtherSection:Toggle({ Name = "Windows", Flag = "Windows", Default = false })
OtherSection:Toggle({ Name = "Filter server ads", Flag = "FilterServerAds", Default = false })
OtherSection:Toggle({ Name = "Filter Console", Flag = "FilterConsole", Default = false })
OtherSection:Toggle({ Name = "Unlock Cvars", Flag = "UnlockCvars", Default = false })
OtherSection:Toggle({ Name = "Fast Reload", Flag = "FastReload", Default = false })
OtherSection:Toggle({ Name = "Fast Weapon Switch", Flag = "FastWeaponSwitch", Default = false })
OtherSection:Toggle({ Name = "Fake Ping", Flag = "FakePing", Default = false })

-- Секция About Neverlose
local AboutSection = MiscPage:Section({
    Name = "About Neverlose",
    Description = "About",
    Icon = "6031091379",
    Side = 2
})
AboutSection:Label("NEVERLOSE.CC"):SetText("NEVERLOSE.CC")
AboutSection:Slider({
    Name = "Dpi Scale",
    Flag = "DpiScale",
    Default = 1,
...
AboutSection:Colorpicker({
    Name = "Style",
    Flag = "StyleColor",
    Default = Color3.fromRGB(255, 255, 255)
})
AboutSection:Keybind({
    Name = "Auto Save",
    Flag = "AutoSaveKey",
...
-- ============================================================
--  ВКЛАДКА CONFIGS (управление конфигами)
-- ============================================================
local ConfigsSection = ConfigsPage:Section({
    Name = "Configs",
...
-- ============================================================
--  ДОПОЛНИТЕЛЬНЫЕ ВКЛАДКИ (заполняем для примера)
-- ============================================================
-- Можно добавить по несколько элементов в каждую вкладку,
-- но для краткости оставим пустые или с минимальным наполнением.

-- Вкладка Aimbot
local AimbotSection = AimbotPage:Section({ Name = "Aimbot", Side = 1 })
AimbotSection:Toggle({ Name = "Enable Aimbot", Flag = "AimbotEnable", Default = false })
AimbotSection:Slider({ Name = "FOV", Flag = "AimbotFOV", Default = 10, Min = 0, Max = 30, Suffix = "°" })

-- Вкладка Ragebot
local RagebotSection = RagebotPage:Section({ Name = "Ragebot", Side = 1 })
RagebotSection:Toggle({ Name = "Enable Ragebot", Flag = "RagebotEnable", Default = false })

-- Вкладка Anti Aim
local AntiAimSection = AntiAimPage:Section({ Name = "Anti Aim", Side = 1 })
AntiAimSection:Toggle({ Name = "Enable Anti Aim", Flag = "AntiAimEnable", Default = false })

-- Вкладка Legitbot
local LegitbotSection = LegitbotPage:Section({ Name = "Legitbot", Side = 1 })
LegitbotSection:Toggle({ Name = "Enable Legitbot", Flag = "LegitbotEnable", Default = false })

-- Вкладка Visuals
local VisualsSection = VisualsPage:Section({ Name = "Visuals", Side = 1 })
VisualsSection:Toggle({ Name = "Enable Visuals", Flag = "VisualsEnable", Default = false })

-- Вкладка Players
local PlayersSection = PlayersPage:Section({ Name = "Players", Side = 1 })
PlayersSection:Toggle({ Name = "Player ESP", Flag = "PlayerESP", Default = false })

-- Вкладка Weapon
local WeaponSection = WeaponPage:Section({ Name = "Weapon", Side = 1 })
WeaponSection:Toggle({ Name = "Weapon ESP", Flag = "WeaponESP", Default = false })

-- Вкладка Grenades
local GrenadesSection = GrenadesPage:Section({ Name = "Grenades", Side = 1 })
GrenadesSection:Toggle({ Name = "Grenade ESP", Flag = "GrenadeESP", Default = false })

-- Вкладка World
local WorldSection = WorldPage:Section({ Name = "World", Side = 1 })
WorldSection:Toggle({ Name = "World ESP", Flag = "WorldESP", Default = false })

-- Вкладка View
local ViewSection = ViewPage:Section({ Name = "View", Side = 1 })
ViewSection:Toggle({ Name = "View ESP", Flag = "ViewESP", Default = false })

-- Вкладка Main
local MainSection = MainPage:Section({ Name = "Main", Side = 1 })
MainSection:Toggle({ Name = "Main Enable", Flag = "MainEnable", Default = false })

-- Вкладка Inventory
local InventorySection = InventoryPage:Section({ Name = "Inventory", Side = 1 })
InventorySection:Toggle({ Name = "Inventory ESP", Flag = "InventoryESP", Default = false })

-- Вкладка Scripts
local ScriptsSection = ScriptsPage:Section({ Name = "Scripts", Side = 1 })
ScriptsSection:Toggle({ Name = "Scripts Enable", Flag = "ScriptsEnable", Default = false })

-- ============================================================
--  КНОПКИ СВЕРНУТЬ / ЗАКРЫТЬ (добавляем вручную)
-- ============================================================
local MainFrame = Window.Items["MainFrame"].Instance
local CloseButton = Window.Items["CloseButton"].Instance
...
MinimizeButton.Position = UDim2.new(1, -56, 0, 11) -- слева от крестика
MinimizeButton.BackgroundTransparency = 0.2
MinimizeButton.BackgroundColor3 = Library.Theme.Element
...
MinimizeIcon.Image = "rbxassetid://6031091379" -- иконка минуса (замените при необходимости)
MinimizeIcon.ImageColor3 = Library.Theme.Text
MinimizeIcon.ZIndex = 3

-- Сделаем кнопку красивой (скругление)
local UICornerMin = Instance.new("UICorner")
UICornerMin.Parent = MinimizeButton
UICornerMin.CornerRadius = UDim.new(0, 7)

-- Добавляем подсветку при наведении (как у других кнопок)
local AccentMin = Instance.new("Frame")
AccentMin.Name = "Accent"
AccentMin.Parent = MinimizeButton
AccentMin.Size = UDim2.new(0, 0, 0, 0)
AccentMin.Position = UDim2.new(0.5, 0, 0.5, 0)
AccentMin.AnchorPoint = Vector2.new(0.5, 0.5)
AccentMin.BackgroundTransparency = 1
AccentMin.BackgroundColor3 = Library.Theme.Accent
AccentMin.ZIndex = 2
local UICornerAccent = Instance.new("UICorner")
UICornerAccent.Parent = AccentMin
UICornerAccent.CornerRadius = UDim.new(0, 7)

MinimizeButton.MouseEnter:Connect(function()
    AccentMin:TweenSize(UDim2.new(1, 0, 1, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quint, 0.15, true)
    AccentMin:TweenProperty("BackgroundTransparency", 0, Enum.EasingDirection.Out, Enum.EasingStyle.Quint, 0.15, true)
end)
MinimizeButton.MouseLeave:Connect(function()
    AccentMin:TweenSize(UDim2.new(0, 0, 0, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quint, 0.15, true)
    AccentMin:TweenProperty("BackgroundTransparency", 1, Enum.EasingDirection.Out, Enum.EasingStyle.Quint, 0.15, true)
end)

-- Плавающая иконка для разворачивания
local FloatingButton = Instance.new("TextButton")
FloatingButton.Name = "FloatingButton"
FloatingButton.Parent = Library.Holder.Instance
...
FloatingButton.BackgroundColor3 = Library.Theme.Background
FloatingButton.BorderSizePixel = 0
FloatingButton.AutoButtonColor = false
FloatingButton.Visible = false
FloatingButton.ZIndex = 127

local FloatingIcon = Instance.new("ImageLabel")
FloatingIcon.Name = "FloatingIcon"
FloatingIcon.Parent = FloatingButton
FloatingIcon.Size = UDim2.new(1, -10, 1, -10)
...
FloatingIcon.Image = "rbxassetid://"..Window.Logo
FloatingIcon.ZIndex = 128

local UICornerF = Instance.new("UICorner")
UICornerF.Parent = FloatingButton
UICornerF.CornerRadius = UDim.new(1, 0)

-- Делаем плавающую иконку перетаскиваемой (используем метод из библиотеки)
-- Но у нас нет объекта Instances, поэтому сделаем вручную
do
    local dragging = false
...
-- По умолчанию водяной знак скрыт, включим его через флаг, если нужно
Library.WatermarkFrame.Instance.Visible = true

-- ============================================================
--  KEYBIND LIST (список активных биндов)
-- ============================================================
local KeybindList = Library:KeybindList("Keybinds")
-- Добавляем в список наш бин из секции About
KeybindList:Add("Auto Save", "F5")
-- Можно добавить и другие

-- Включаем отображение списка (по умолчанию скрыт)
KeybindList:SetVisibility(true)

-- ============================================================
--  УВЕДОМЛЕНИЕ (при нажатии кнопки)
-- ============================================================
-- Добавим кнопку, которая показывает уведомление, например, в вкладке Configs
ConfigsSection:Button({
    ...
    Callback = function()
        Library:Notification({
            ...
            Duration = 3,
            IconColor = {
                Start = Library.Theme.Accent,
                End = Library.Theme.AccentGradient
            }
        })
    end
})
...
-- ============================================================
-- Окно уже отображается (Window:SetOpen(true) вызывается внутри Library.Window)
-- Дополнительно можно настроить позицию
Window:SetCenter()

print("GUI успешно загружен! Нажмите Insert для открытия/закрытия (по умолчанию).")
