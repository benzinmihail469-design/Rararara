...
-- Добавьте этот код внутрь контекста Library, где определяются элементы (например, в Library.Sections)
function Library.Sections:AddButton(Data)
    local Button = {
    ...
    local ButtonFrame = Instances:Create("TextButton", {
        Parent = self.Container, -- Ваш фрейм секции
        Name = "\0",
    ...
    }) ButtonFrame:AddToTheme({TextColor3 = "Text", BackgroundColor3 = "Element"})
...
function Library.Sections:AddSlider(Data)
    local Slider = {
    ...
    local SliderBar = Instances:Create("TextButton", {
        Parent = SliderFrame.Instance,
        Name = "\0",
        ...
        Text = "",
        AutoButtonColor = false
    }) SliderBar:AddToTheme({BackgroundColor3 = "Outline"})
    ...
    local function Update(Input)
        local SizeX = math.clamp((Input.Position.X - SliderBar.Instance.AbsolutePosition.X) / SliderBar.Instance.AbsoluteSize.X, 0, 1)
        SliderFill:Tween(TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(SizeX, 0, 1, 0)})
        local Value = math.floor(Slider.Min + ((Slider.Max - Slider.Min) * SizeX))
        SliderText.Instance.Text = Slider.Name .. " (" .. Value .. ")"
