...
local function StartFlying()
    ...
    FlyConnection = RunService.RenderStepped:Connect(function()
        if not Flying then StopFlying() return end
        ...
        BVelocity.Velocity = moveDir.Magnitude > 0 and moveDir.Unit * FlySpeed or Vector3.new(0,0,0)
    end)
end

-- ==========================================
-- БЕЗОПАСНЫЙ АВТО-ФАРМ (БЕЗ ТЕЛЕПОРТАЦИИ)
-- ==========================================

local function GetTargetCoinGlobal()
    if CachedCoin and CachedCoin.Parent and CachedCoin.Transparency < 1 then return CachedCoin end
...
local function StartAutoFarm()
    ...
    AutoFarmConnection = RunService.Heartbeat:Connect(function()
        ...
        if coin then
            ...
            if dist > 3 then
                -- Плавное движение к монете вместо телепортации
                root.Velocity = (coin.Position - root.Position).Unit * 120
            else
...
CreateToggle(MainTab, "Safe AutoFarm", false, function(state)
...
CreateSlider(PlayerTab, "Fly Speed", 15, 90, 35, function(value) FlySpeed = value end)
