...
-- ==================== ФУНКЦИЯ ИЗМЕНЕНИЯ СКОРОСТИ (С ОБХОДОМ АНТИЧИТА) ====================
...
local function UpdateSpeed()
    ...
    if Settings.SpeedEnabled then
        -- Устанавливаем реальную скорость
        humanoid.WalkSpeed = Settings.SpeedValue
        
        -- Если обход ещё не установлен, активируем его
        if not oldSpeedIndex then
            oldSpeedIndex = hookmetamethod(game, "__index", function(self, key)
                if self == humanoid and key == "WalkSpeed" then
                    return 16 -- Показываем стандартную скорость при проверке
                end
                return oldSpeedIndex(self, key)
            end)
        end
    else
        -- Отключаем обход и сбрасываем скорость
        if oldSpeedIndex then
            pcall(function() oldSpeedIndex = nil end)
        end
        humanoid.WalkSpeed = 16
    end
end

-- ==================== ФУНКЦИЯ БЕСКОНЕЧНОЙ СТАМИНЫ (С ОБХОДОМ АНТИЧИТА) ====================
local StaminaHook = nil
local StaminaLoop = nil

local function EnableInfiniteStamina()
    if StaminaHook then
        pcall(function() StaminaHook = nil end)
    end
    if StaminaLoop then
    ...
    -- Способ 1: Перехват запросов стамины через __index
    StaminaHook = hookmetamethod(game, "__index", function(self, key)
        if self == LocalPlayer and (key == "Stamina" or key == "stamina" or key == "Energy" or key == "energy" or key == "Endurance" or key == "endurance") then
            return 100
        end
        return StaminaHook(self, key)
    end)
    
    -- Способ 2: Установка атрибутов каждый кадр
    ...
    -- Способ 3: Поиск и блокировка RemoteEvent стамины
...
print("ESP + Speed 30 + Infinite Stamina loaded! (Anti-Cheat Bypass)")
