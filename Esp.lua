...
-- Р¤СѓРЅРєС†РёСЏ РїСЂРѕРІРµСЂРєРё - РІ РёРіСЂРµ Р»Рё РёРіСЂРѕРє (РµСЃС‚СЊ Р»Рё Сѓ РЅРµРіРѕ РѕСЂСѓР¶РёРµ/РёРЅСЃС‚СЂСѓРјРµРЅС‚С‹)
local function IsPlayerInGame(player)
    if not player.Character then return false end
    local hasTool = false
    
    -- РџСЂРѕРІРµСЂСЏРµРј РµСЃС‚СЊ Р»Рё Сѓ РёРіСЂРѕРєР° РѕСЂСѓР¶РёРµ (РІ СЂСѓРєРµ РёР»Рё РІ СЂСЋРєР·Р°РєРµ)
    ...
    -- Р•СЃР»Рё Сѓ РёРіСЂРѕРєР° РЅРµС‚ РѕСЂСѓР¶РёСЏ - СЃРєРѕСЂРµРµ РІСЃРµРіРѕ РѕРЅ РІ Р»РѕР±Р±Рё/СЃРїРµРєС‚Р°С‚РѕСЂРµ
    return hasTool
end

-- РРЎРџР РђР’Р›Р•РќРќР«Р™ ESP - РџРћРљРђР—Р«Р’РђР•Рў РЎРљР’РћР—Р¬ РЎРўР•РќР«
task.spawn(function()
...
-- Auto Kill РґР»СЏ РњРђРќР¬РЇРљРђ (СѓР±РёРІР°РµС‚ РўРћР›Р¬РљРћ Р°РєС‚РёРІРЅС‹С… РёРіСЂРѕРєРѕРІ РІ РёРіСЂРµ, РЅРµ С‚СЂРѕРіР°РµС‚ Р»РѕР±Р±Рё)
task.spawn(function()
    while task.wait(0.25) do
        if AutoKillEnabled then
            ...
            if not isMurderer then
                -- Р•СЃР»Рё РјС‹ РЅРµ РјР°РЅСЊСЏРє - РЅРёС‡РµРіРѕ РЅРµ РґРµР»Р°РµРј
            elseif knife and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                for _, target in pairs(Players:GetPlayers()) do
                    if target ~= LocalPlayer and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        -- РџСЂРѕРІРµСЂСЏРµРј: С†РµР»СЊ Р¶РёРІР°, РќР• РјР°РЅСЊСЏРє, Р РІ РёРіСЂРµ (РµСЃС‚СЊ РѕСЂСѓР¶РёРµ)
...
-- Auto Shoot Murderer РґР»СЏ РЁР•Р РР¤Рђ (СѓР±РёРІР°РµС‚ РўРћР›Р¬РљРћ РјР°РЅСЊСЏРєР°, РєРѕС‚РѕСЂС‹Р№ РІ РёРіСЂРµ)
task.spawn(function()
    while task.wait(0.2) do
                        ...
                        for _, target in pairs(Players:GetPlayers()) do
                            if target ~= LocalPlayer then
                                ...
                                if targetIsMurderer and target.Character then
                                    ...
                                    -- РџСЂРѕРІРµСЂСЏРµРј: РјР°РЅСЊСЏРє Р¶РёРІ Рё РІ РёРіСЂРµ
...
local MainTab = CreateTab("Р“Р»Р°РІРЅР°СЏ", 1)
local PlayerTab = CreateTab("РРіСЂРѕРє", 2)
local VisualTab = CreateTab("Р’РёР·СѓР°Р»", 3)
local KillerTab = CreateTab("РљРёР»Р»РµСЂ", 4)
local SheriffTab = CreateTab("РЁРµСЂРёС„", 5) 

CreateToggle(MainTab, "РЈРЅРёРІРµСЂСЃР°Р»СЊРЅС‹Р№ РђРІС‚Рѕ-Р¤Р°СЂРј РњРѕРЅРµС‚", false, function(state)
    AutoFarmEnabled = state
    if state then StartAutoFarm() else StopAutoFarm() end
end)

CreateSlider(MainTab, "РЎРєРѕСЂРѕСЃС‚СЊ Р°РІС‚Рѕ-С„Р°СЂРјР°", 10, 25, 16, function(value)
    AutoFarmSpeed = value
end)
...
CreateSlider(PlayerTab, "РЎРєРѕСЂРѕСЃС‚СЊ РїРѕР»РµС‚Р°", 15, 90, 35, function(value)
    FlySpeed = value
end)
...
CreateToggle(PlayerTab, "Noclip (РЎРєРІРѕР·СЊ СЃС‚РµРЅС‹)", false, function(state)
    if state then
...
CreateToggle(VisualTab, "MM2 ESP (Р РѕР»Рё)", false, function(state)
    ESPEnabled = state
end)

CreateToggle(KillerTab, "Auto Kill (Р’СЃРµС…)", false, function(state)
    AutoKillEnabled = state
end)

CreateToggle(SheriffTab, "РђРІС‚Рѕ-РЈР±РёР№СЃС‚РІРѕ РјР°РЅСЊСЏРєР°", false, function(state)
    AutoShootMurdererEnabled = state
end)

CreateToggle(SheriffTab, "РђРІС‚Рѕ-РїРѕРґР±РѕСЂ РїРёСЃС‚РѕР»РµС‚Р° (РќРµРІРёРЅРѕРІРЅС‹Рј)", false, function(state)
    AutoGetGunEnabled = state
end)

if tabs["Р“Р»Р°РІРЅР°СЏ"] then
    tabs["Р“Р»Р°РІРЅР°СЏ"].Select()
end
